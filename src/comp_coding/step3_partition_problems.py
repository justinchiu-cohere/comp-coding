...
def problem_to_sft_samples(problem: Problem) -> List[SFTSample]:
    ...
    Uses the problem's prompt and each sample's full r1_generation as completion.
    """
    sft_samples = []
    for sample in problem.samples:
        # Use r1_generation as the full completion if available, otherwise fall back to solution
...
def create_aggregated_training_splits(
    ...
    stats = {
        ...
        "partitions": list(range(1, num_partitions + 1)),
        "problems": len(rl_samples_with_completions),
